...
import {ScrollView,Image, TouchableOpacity, Text, View, StyleSheet,TextInput,Pressable,FlatList} from 'react-native';
...
function Cart({navigation,route}){
 ...
 const data = await response.json();
  ...
  const userId=route.params.data.customerid; //logged in user
   ...
   // c
    if(userId==id){
      setcart(prev=>([...prev,{name:data[obj].name, price:data[obj].price, image: data[obj].image,pID: data[obj].pID }]))
...
<View>
 <FlatList
        ...
        renderItem={({ item }) => (
          ...
          <Text style={{fontWeight:"bold",fontSize:20,marginLeft:5}}>{item.name} </Text>
          </View>
         <Text style={{fontSize:18,marginLeft:70,marginTop:-25}}>Price: {item.price} </Text>
           <TouchableOpacity
        style={{ backgroundColor: '#00A651', height: 35 ,width:35,borderRadius:15,justifyContent:"center",marginLeft:250,marginTop:-10}}>
   ...
   <TouchableOpacity
        style={{ backgroundColor: '#00A651', height: 40 ,width:150,marginTop:20,justifyContent:"center",alignSelf:"center"}}
        onPress={() =>
